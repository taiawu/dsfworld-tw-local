...
```
```{r}
tm_table_all_s1 <- readRDS("../../0_integrated_modules/values_df_tm_models_table.rds")
tm_table_all<- readRDS("../../0_integrated_modules/values_df_tm_models_s1.rds")

tm_table_all
tm_table_all_s1
```
```{r}
tm_table_all <- readRDS("../../0_integrated_modules/values_s2_list.rds")
tm_table_all$df_models_sgd1 %>%
  recode()
...
tm_table_all$df_BIC
# values_s2_d_list.rds
# values_s1_list.rds
# values_s1_d_list.rds
# values_model_list.rds
# values_df_tm_models.rds
```
...
make_wide_preds( df_models ) 
              
```

```{r}
df_best_model <- readRDS("../../0_integrated_modules/values_df_tm_models_table.rds")
df_best_model
```
...
```{r}
start_pars <- readRDS("../../0_integrated_modules/values_start_pars.rds")
df_fit <- readRDS("../../0_integrated_modules/values$df_fit.rds")
which_model <- s1_model
model_name <- "s1_pred"
win3d <- 7
...
model_all(s1_model, "s1_pred", start_pars, win3d)
```
...
```{r}
all(c("condition", "Temperature", "mean", "sd") %in% c("condition", "Temperature", "mean", "sd")) 
```
...
```{r}
  df %>%
    ...
    mutate(well_f_ = factor(.$well_, levels = make_well_names("ROWS", "1"))) 
```
...
```{r}
parse_number("ABC1234")
str_extract_all("ABC1234")

well_vec <- c("A1", "B2", "whatthiswhy", "1345")
num <- parse_number(well_vec) %>% as.character() %>% replace_na(replace = "1") 
str_remove(well_vec, num)
```
...
```{r}
data_raw <- read_csv("../../0_integrated_modules/sample_file.csv") %>% nest_raw()
...
parse_well_vec(well_vec)$row
parse_well_vec(well_vec)$col

parse_well_vec(data_raw$well)
```




![Caption for the picture.](sample_pic.png)
